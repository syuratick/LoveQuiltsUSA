const AboutLoveQuilt = () => {
  return (
    <main className="about-page">
      <div className="container py-5">
        <div className="about-content text-center">
          <h1 className="about-heading">Welcome to Love Quilts USA</h1>

          <p className="about-tagline">
            Beautifully crafted quilts for children with life-threatening illnesses.
          </p>

          <p className="about-text">
            Our quilts are made with hand-cross stitched squares that are sent to
            us by the greatest bunch of caring individuals from all over the
            world. The quilts are assembled by equally caring and devoted women.
          </p>

          <p className="about-text">
            Please take time to look around … explore using the links provided on
            the left. Perhaps you'll be inspired to stitch one quilt square or
            many. Perhaps you'll decide to help us by assembling quilts. Or
            perhaps you have or know of a sick child who could benefit by being
            covered in the love these quilts provide.
          </p>

          <p className="about-text">
            If you would like more information, please email Nezzy at{' '}
            <a href="mailto:nezzylq@gmail.com" className="about-email">
              nezzylq@gmail.com
            </a>
          </p>

          <p className="about-thanks">Thank you for joining us here!!</p>
        </div>
      </div>
    </main>
  );
};

export default AboutLoveQuilt;
